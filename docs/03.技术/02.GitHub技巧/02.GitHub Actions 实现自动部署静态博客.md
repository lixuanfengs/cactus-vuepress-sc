...
一开始的部署方式是使用[sh部署脚本](https://github.com/lixuanfengs/cactus-vuepress-sc/blob/master/deploy.sh)把代码提交到这两个平台的仓库分支，虽然已经很方便了，但是我还想把博客未打包的源码提交到Github主分支上。这就需要我操作两次命令，**我就想能不能只需要一次操作就可以同时把源码、部署代码一次性提交到两个平台呢？**
...
![token设置](https://cdn.jsdelivr.net/gh/lixuanfengs/blog-images/cactus-vuepress-img/_2022049151048.png 'token设置')

GitHub Actions 的配置文件叫做 workflow 文件，存放在代码仓库的`.github/workflows`目录。

workflow 文件采用 [YAML 格式](https://cactuslixf.tk/pages/4e8444e2d534d14f/)，文件名可以任意取，但是后缀名统一为`.yml`，比如`ci.yml`。一个库可以有多个 workflow 文件。GitHub 只要发现`.github/workflows`目录里面有`.yml`文件，就会自动运行该文件。    
...
# deploy to coding
echo 'www.cactuslixf.com\cactuslixf.com' > CNAME  # 自定义域名
if [ -z "$CODING_TOKEN" ]; then  # -z 字符串 长度为0则为true；$CODING_TOKEN来自于github仓库`Settings/Secrets`设置的私密环境变量
  codingUrl=git@git.dev.tencent.com:lixuanfengs/cactus-vuepress.git
else
  codingUrl=https://lixuanfengs:${CODING_TOKEN}@git.dev.tencent.com/lixuanfengs/cactus-vuepress.git
fi
git add -A
git commit -m "${msg}"
