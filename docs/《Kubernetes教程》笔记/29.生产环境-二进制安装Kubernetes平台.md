---
title: 生产环境-二进制安装Kubernetes平台
date: 2022-05-09 10:35:58
permalink: /pages/6f3bc4/
categories:
  - 后端
  - Kubernetes学习笔记
tags:
...
![ha-master-gce](https://cdn.jsdelivr.net/gh/lixuanfengs/blog-images/cactus-vuepress-img/ha-master-gce.png)
...
![img](https://cdn.jsdelivr.net/gh/lixuanfengs/blog-images/cactus-vuepress-img/u=1138484328,1635358857&fm=15&gp=0.jpg)
...
# 每个机器准备域名
vi /etc/hosts
192.168.1.6 k8s-master1
192.168.1.7 k8s-master2
192.168.1.11 k8s-master3
192.168.1.12 k8s-node1
192.168.1.8 k8s-node2
192.168.1.9 k8s-node3
192.168.1.16 k8s-master-lb # 非高可用，可以不用这个。这个使用keepalive配置
...
![1621419733778](https://cdn.jsdelivr.net/gh/lixuanfengs/blog-images/cactus-vuepress-img/1621419733778.png)
...
wget https://github.com/etcd-io/etcd/releases/download/vv3.5.2/etcd-v3.5.2-linux-amd64.tar.gz
...
cfssl gencert \
   ...
   -profile=etcd \
   etcd-cactusli-csr.json | cfssljson -bare /etc/kubernetes/pki/etcd/etcd
```
...
listen-client-urls: 'https://192.168.1.6:2379,http://127.0.0.1:2379'
max-snapshots: 3
max-wals: 5
cors:
...
discovery-proxy:
discovery-srv:
initial-cluster: 'etcd-master1=https://192.168.1.6:2380,etcd-master2=https://192.168.1.7:2380,etcd-master3=https://192.168.1.11:2380'
initial-cluster-token: 'etcd-k8s-cluster'
initial-cluster-state: 'new'
strict-reconfig-check: false
enable-v2: true
...
election-timeout: 1000
quota-backend-bytes: 0
listen-peer-urls: 'https://192.168.1.7:2380'
listen-client-urls: 'https://192.168.1.7:2379,http://127.0.0.1:2379'
max-snapshots: 3
max-wals: 5
cors:
initial-advertise-peer-urls: 'https://192.168.1.7:2380'
advertise-client-urls: 'https://192.168.1.7:2379'
discovery:
discovery-fallback: 'proxy'
discovery-proxy:
discovery-srv:
...
```sh
# 查看etcd集群状态
etcdctl --endpoints="192.168.1.6:2379,192.168.1.7:2379,192.168.1.11:2379" --cacert=/etc/kubernetes/pki/etcd/ca.pem --cert=/etc/kubernetes/pki/etcd/etcd.pem --key=/etc/kubernetes/pki/etcd/etcd-key.pem  endpoint status --write-out=table
...
## 更多etcdctl命令，https://etcd.io/docs/v3.5/demo/
```
...
![1621403784124](https://cdn.jsdelivr.net/gh/lixuanfengs/blog-images/cactus-vuepress-img/1621403784124.png)
...
// 192.168.1.16： 是我准备的负载均衡器地址（负载均衡可以自己搭建，也可以购买云厂商lb。）
{
    "CN": "kube-apiserver",
    "hosts": [
      "10.96.0.1",
...
kubectl config set-cluster kubernetes \
     ...
     --embed-certs=true \
     --server=https://192.168.1.16:8443 \
...
>   - 添加后端服务器地址与端口
>
>   ![1621488258001](https://cdn.jsdelivr.net/gh/lixuanfengs/blog-images/cactus-vuepress-img/1621488258001.png)
...
[root@k8s-master3 kubernetes]# cat /etc/haproxy/haproxy.cfg
global
  maxconn  2000
  ulimit-n  16384
...
backend k8s-maste
  ...
  server k8s-master1    192.168.1.6:6443  check
  server k8s-master2    192.168.1.7:6443  check
  server k8s-master3    192.168.1.11:6443  check

  
#配置keepalived
#k8s-master1
[root@k8s-master1 k8sworker]#  cat /etc/keepalived/keepalived.conf
...
#k8s-master2
[root@k8s-master2 kubernetes]# cat /etc/keepalived/keepalived.conf
! Configuration File for keepalived
global_defs {
    router_id LVS_DEVEL
}
...
#k8s-master3
[root@k8s-master3 kubernetes]# cat /etc/keepalived/keepalived.conf
! Configuration File for keepalived
global_defs {
    router_id LVS_DEVEL
}
...
systemctl daemon-reload && systemctl enable haproxy && systemctl enable keepalived

#测试是否配置成功，查看虚拟节点上的网卡是否多出 master 节点的IP 
[root@k8s-lb ~]# ip a 
...
ExecStart=/usr/local/bin/kube-apiserver \
      ...
      --etcd-servers=https://192.168.1.6:2379,https://192.168.1.7:2379,https://192.168.1.11:2379 \
...
#所有证书复制文件到全部节点上 所有node节点必须有 kubelet kube-proxy
...
Environment="KUBELET_CONFIG_ARGS=--config=/etc/kubernetes/kubelet-conf.yaml --pod-infra-container-image=registry.cn-hangzhou.aliyuncs.com/cactusmall/pause:3.6"
...
sed -i 's#etcd_endpoints: "http://<ETCD_IP>:<ETCD_PORT>"#etcd_endpoints: "https://192.168.1.6:2379,https://192.168.1.7:2379,https://192.168.1.11:2379"#g' calico.yaml
...
![1621514630209](https://cdn.jsdelivr.net/gh/lixuanfengs/blog-images/cactus-vuepress-img/1621514630209.png)
...
kubectl label node k8s-master2 node-role.kubernetes.io/master='' node-role.kubernetes.io/work=''
kubectl label node k8s-master3 node-role.kubernetes.io/master='' node-role.kubernetes.io/work=''
kubectl label node k8s-node1 node-role.kubernetes.io/work=''
kubectl label node k8s-node2 node-role.kubernetes.io/work=''
kubectl label node k8s-node3 node-role.kubernetes.io/work=''

#设置污点
kubectl taint nodes k8s-master1 node-role.kubernetes.io/master=true:NoSchedule
#删除污点
kubectl taint nodes k8s-master1 node-role.kubernetes.io/master=-
```
